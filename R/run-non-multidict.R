# ------------------------------------------------------------------------------
#
# Script name: run-non-multidict.R
#
# Purpose of script: Function to run a regular dictionary in the same way as
# the run_multidict function.
#
...
run_non_multidict <- function(
  ...
  if (!(pattern_type %in% c("regex", "glob", "fixed"))) {
    stop('pattern_type has to be one of "regex", "glob", or "fixed".')
  }
  # Check return_value
  ...
  if ((regex_optimize | regex_make_greedy | regex_make_lazy) &
    (pattern_type == "glob" | pattern_type == "fixed")) {
    message('Regex options are ignored for pattern_type "fixed " or "glob".')
  }
